...
- **KiCAD-Compatible PCB Editor**: Our custom-built PCB editor reads and writes native `.kicad_pcb` files. Built as a fork of KiCanvas with a full editing layer — component drag-and-drop, footprint rotation/flip, board outline editing (rectangle or SVG import), interactive trace routing, zone drawing, layer management, ratsnest display, courtyard DRC, undo/redo, and a 3D board preview. Everything stays in the KiCAD s-expression format so you can open your boards directly in KiCAD.
  ...
  > **Planned migration → `sparkbench-canvas@editor-v1.3`** (2026-06-11; v1.0 GA 2026-06-10): the in-app fork above is superseded by the [sparkbench-canvas](https://github.com/sparkbencheda/sparkbench-canvas) v2 editor. It covers the full parity checklist (footprint placement + courtyard DRC, routing, vias/zones, board outline rect + SVG → `Edge.Cuts`, ratsnest, 50-step undo) behind the same integration shape we use today — `<sparkbench-editor>` web component, `pcbText` in → `save` event with serialized text out — plus a byte-identical round-trip serializer gated against KiCad 10 and a DOM-free `./core` export so Sparky's server-side board mutation (MCP tools / `pcb-pipeline`) can run the same engine headless in Node. `editor-v1.1` adds the daily-driver layer: KiCad-style click disambiguation (vias selectable inside pours, click-through cycling), true-shape selection highlight, a selection-filter panel, a net inspector + backtick net-highlight, user-settable track-width/via-size (toolbar dropdowns or `editor.session`), and full `.kicad_pro` net-class support (`editor.projectText = proText` drives routing widths, via specs, and per-class DRC clearances; `parseProjectSettings`/`netClassFor` are exported headless from `./core`). `editor-v1.2` adds the agent surface Sparky's MCP tools want: an **edit-operations API** — `moveFootprint`/`placeFootprint`/`routeTrack`/`deleteItems` (+ sch `placeSymbol`/`drawWire`/`setProperty`) each return an act→consequences report (DRC/courtyard violations introduced *and* resolved, per-net unconnected deltas, changed uuids, `undo()`) — available headless from `./core` *and* live on the element via `await editor.applyOperation({kind:"moveFootprint", refOrUuid:"R5", to:{x,y}})` with JSON-serializable reports (replaces `CheckFloorplan`-style hand-rolled AABB checks); plus a layers panel (active routing layer + per-layer visibility), grid-size control, zoom-to-fit (Home), board text editing (footprint Reference/Value, silkscreen), and a `footprintProvider` hook for part-DB-backed footprint placement. `editor-v1.3` closes the design-from-scratch loop this repo needs to retire its fork entirely: a `<sparkbench-project>` element hosting sch+pcb behind tabs with **forward annotation** (Update PCB from Schematic — exact-reference netlist sync placing footprints via the provider, rewriting pad nets, dry-run preview, one-step undo; also headless as `forwardAnnotate` in `./core`) and two-way cross-probe; KiCad-style sch moves (drag-with-wires, R/X/Y, multi-unit placement, free text); thermal-relief zone fills with **zone-aware connectivity** (a GND pour actually clears ratsnest) and auto-refill; editable copper stackup (1–32 layers, so 4-layer boards work end to end); and **fab outputs** — native `bomOf`/`pickAndPlaceOf` (kicad-cli-compatible CSV) in `./core` plus a Node-only `./fab-cli` kicad-cli wrapper for Gerbers/drill, replacing the hand-rolled export paths in `pcb-pipeline`. See `docs/guides/PROJECT.md` + `docs/guides/FAB-OUTPUTS.md` in that repo.
