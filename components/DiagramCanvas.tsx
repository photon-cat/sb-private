...
import { useSimulationWiring } from "@/hooks/useSimulationWiring";
import {
  renderWires,
  pathToHints,
  type RenderedWire,
...
function ensureElementsLoaded(): Promise<void> {
  ...
  elementsLoaded = true;
  return import("@sparkbench/elements").then(() => {
    registerDipChips();
    registerLogicGates();
  });
}
...
export default function DiagramCanvas({
  ...
  const { sensorEntries } = useSimulationWiring({
    runner,
    diagram,
    elementsRef,
    chipConfigs,
        ...
        requestAnimationFrame(() => {
          ...
          setWires((current) => {
            ...
            for (const w of current) {
              ...
              if (!conn) continue;
              updater(w.connectionIndex, { ...conn, hints });
            }
            return current;
          });
        });
      ...
      if (ds && preview) {
        ...
        if (w && diagram) {
          ...
          if (conn) {
            ...
            const newConn: DiagramConnection = { ...conn, hints };
