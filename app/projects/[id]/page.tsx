...
export default function ProjectPage() {
  ...
  // Fetch star status on mount
  useEffect(() => {
    fetchStarStatus(projectId).then((data) => setStarred(data.starred));
  }, [projectId]);
  ...
  // Load diagram and sketch on mount (or when projectId changes)
  useEffect(() => {
    if (!projectId) return;
  ...
  const handleLibrariesChange = useCallback(async (text: string) => {
    ...
    if (projectId) {
      try {
        await saveLibraries(projectId, text);
      } catch (err) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      const promises: Promise<void>[] = [
        saveDiagram(projectId, diagramJson),
        saveSketch(projectId, sketchCode, projectFiles),
      ];
      if (pcbText !== null) {
        promises.push(savePCB(projectId, pcbText));
      }
      if (librariesTxt) {
        promises.push(saveLibraries(projectId, librariesTxt));
      }
  ...
  }, [projectId, diagramJson, sketchCode, pcbText, projectFiles, librariesTxt]);
  ...
  const handleDownloadZip = useCallback(async () => {
    ...
    a.click();
    URL.revokeObjectURL(url);
  }, [diagramJson, sketchCode, projectFiles, pcbText, projectId]);
  ...
  const handleToggleVisibility = useCallback(async () => {
    ...
    try {
      ...
      setProjectSettings((prev) => prev ? { ...prev, isPublic: newValue } : prev);
    } catch (err) {
  ...
  const handleRenameProject = useCallback(async (newName: string) => {
    ...
    try {
      ...
      setProjectSettings((prev) => prev ? { ...prev, title: newName } : prev);
    } catch (err) {
